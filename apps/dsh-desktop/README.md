...
把 dsh 封装成不依赖外部浏览器的桌面应用（Wails v3 壳；macOS / Linux / Windows）。

## 架构

```
DSH.app/Contents/          （macOS 布局；Linux/Windows 产物布局见根 README「构建说明」）
...
指向新地址。应用退出（窗口关闭 / Cmd+Q / 外部 SIGTERM）时终止后端：Unix 上 SIGTERM
其进程组（含 SEA 内嵌 node 的 dsh-server，及将来可能 spawn 的子进程），
宽限期内未退出则 SIGKILL 兜底；Windows 上经 Job Object（`KILL_ON_JOB_CLOSE`）
按作业树终止（Windows 无 POSIX 信号，SEA node 收不到优雅 SIGTERM，直接
强杀全树）。确保不留孤儿 node；main 会等守护协程收口后才真正退出。壳进程注册了 SIGTERM/SIGINT 处理：外部 kill 不会
...
GTK3 分支默认 OnDemand），保证渲染走 GPU 合成而非 CPU；Windows 用系统
WebView2（Edge 运行时，Win10 1803+ 自带），壳以 GUI 子系统构建
（`-ldflags "-H=windowsgui"`），后端以 `CREATE_NO_WINDOW` 启动，均不弹
控制台黑窗。
...
server/       子包：SEA 后端进程生命周期（Start / Process.Stop / Exit、
              ...
              平台差异收在 server_unix.go（Setpgid + 进程组信号）与
              server_windows.go（Job Object + CREATE_NO_WINDOW + .exe 名）
```
...
just build-windows-app   # Windows 完整构建（须在 Windows 主机上执行，见根 README）
```
...
后端（dsh-server）继承的环境变量：Unix 上启动前按 `$SHELL` source 用户 shell
配置（bash → `~/.bashrc`，zsh → `~/.zshrc`），使后端继承用户终端里 export 的
变量；source 输出重定向到 /dev/null，不污染后端 stdout；用 `exec` 保持
同一进程（PID 不变），守护 wait 语义不受影响。Windows 上无 shell rc 概念，
直接 exec 后端（进程仍入 Job Object 以便退出收口）。常用透传变量：
