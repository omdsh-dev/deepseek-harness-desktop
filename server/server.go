...
//
// dsh-server 是桌面应用壳同目录下的 SEA 可执行（内嵌 node 的 dsh CLI），
// 启动参数 `--profile <name> --port <port>`（端口传 "0" 由 OS 分配，避免
// 冲突）。就绪后 web-app 插件把 `dsh web: http://127.0.0.1:<port>` 打到
// stdout，本包解析该行得到实际监听地址。终止语义平台相关：Unix 按独立
// 进程组发信号（Setpgid），Windows 用 Job Object 按作业树终止
// （CREATE_NEW_PROCESS_GROUP），实现见 server_unix.go / server_windows.go。
package server
...
// port 传 "0" 时由 OS 分配（默认，避免与已占用端口冲突）。dshHome 非空时
// 注入 DSH_HOME 环境变量。ctx 取消或超时时终止后端并返回错误。成功返回的
// Process 由调用方在退出/重启时终止。
